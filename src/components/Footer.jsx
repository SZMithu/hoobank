    ...
    <section className={`${styles.flexCenter} ${styles.paddingY
      ...
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
        <div className="flex-1 flex flex-col justify-start mr-10">
          ...
          <p className={`${styles.paragraph} mt-4 max-w-[310px] text-start`}>
            ...
            <div key={index} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
                  ...
                  <ul>
                    {footerLink.links.map((link,idx) => (
                      <li key={idx} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${idx !== footerLink.links
                      .length - 1 ? "mb-3" : "mb-0"}`}>{link.name}</li>
                    ))}
                  </ul>
            </div>
          ))}
